function templateMenu(category, index){
    let dish = dishes[category][index];
    return `
        <div class="dish">
...
function templateCartItem(item, index, sum){
    return `
     <div class="cartDishContainer" id="cartDish-${item.category}-${index}">
        <span>${item.name}</span>
        <div class="cart-dish-area">
            ...
            <span id="amount-${item.category}-${index}">${item.amount}</span>
            <button 
            ...
            <span id="sum-${item.category}-${index}" class="sum-style-container">${sum} €</span>
            <button 
                id="remove-${item.category}-${index}"
